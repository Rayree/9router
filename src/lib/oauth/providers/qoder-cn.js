...
const qoderCn = {
  ...
  // Qoder CN uses the same device flow as international Qoder:
  // PKCE + nonce + machine_id are generated locally, the user lands on
  // qoder.cn/device/selectAccounts in the browser, and we poll
  // qoder.cn/api/v1/deviceToken/poll until a `dt-...` token appears.
  requestDeviceCode: async (config) => {
    const { QoderCnService } = await import("@/lib/oauth/services/qoderCn");
    const flow = new QoderCnService().initiateDeviceFlow();
  ...
  pollToken: async (config, deviceCode, codeVerifier, extraData) => {
    ...
    // expireTime is a Unix-ms timestamp from QoderCnService.parseExpiry,
